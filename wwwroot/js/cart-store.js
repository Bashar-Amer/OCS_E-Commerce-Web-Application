...
const BarrameruStore = {
    ...
    async addToCart(productId, qty = 1, metadata = null, btnElement = null) {
        ...
        // Set this in your Layout view: window.isUserLoggedIn = @User.Identity.IsAuthenticated.ToString().ToLower();
        const isLogged = window.isUserLoggedIn || false;

        if (isLogged) {
            // --- LOGGED-IN: SEND TO BACKEND DATABASE ---
            try {
